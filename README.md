...
品牌标识“鹿鸣”由梅花鹿、鹿角与梅花斑点组成。桌面小鹿在湖岸饮水，并以三秒为周期温柔回头，不会一直注视使用者；动画背景完全透明。
...
- 每日查询的英文词自动进入“晨读”清单（去重、每天最多 20 个），支持逐词发音与完成勾选
- 晨读清单可按时间戳归档到 `C:\LLMWiki\LLMWiki\wiki\sources\LinguaOrb`，同时保存 Markdown 内容与逐词 MP3 音频
- “复习”页按时间倒序展示全部历史晨读清单，点击任意一次即可加载回晨读列表
- 页面顶部实时显示 UDP/TCP 当前请求数与本次启动累计请求数（`UDP:n/M  TCP:n/M`）
- 常用示例词支持断网查询
- 无边框、置顶、可拖动的悬浮卡片
- 原创卡通猫头鹰学习伙伴
